...
//===== Subtract by Vector2 =====
///	Vector2 subtraction. This is done componentwise. 2D check done before z is changed. 
///	\param vec is the Vector2 subtracted from the new Vector2.
        TEMPEST_API Vector2 operator-(const Vector2& vec) const;

///	Vector2 subtraction. This is done componenetwise. 2D check done before z is changed. 
///	\param vec is the Vector2ed subtractd from this Vector2.
        TEMPEST_API Vector2& operator-=(const Vector2& vec);

//===== Negation and increment =====
///	Changes the sign of each element of the Vector2. If 2D, z is not changed.
        inline Vector2 operator-(void)
        {
            return Vector2(-_data[x], -_data[y]);
        }

///	Prefix, Adds 1 to each element of the Vector2. If 2D, z is ignored. 
        inline Vector2& operator++(void)
        {
            ++_data[x];
            ++_data[y];
...
///	Postfix, Adds 1 to each element of the Vector2. If 2D, z is ignored. 
        inline Vector2 operator++(int)
        {
            return Vector2(++_data[x], ++_data[y]);
        }

///	Prefix, Subtracts 1 to each element of the Vector2. If 2D, z is ignored. 
        inline Vector2& operator--(void)
        {
            --_data[x];
            --_data[y];
...
///	Postfix, Subtracts 1 to each element of the Vector2. If 2D, z is ignored.  
        inline Vector2 operator--(int)
        {
            return Vector2(--_data[x], --_data[y]);
        }
...
///	Vector2 multiplication. This is a componentwise multiplication, scaling one Vector2 by another. 2D check done before z is changed.
...
//===== Mutliply by Scalar =====
///	Scalar multiplication. This is a componentwise multiplication, scaling the Vector2 by the scalar. 2D check done before z is changed.
