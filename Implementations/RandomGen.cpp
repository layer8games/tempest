...
namespace KillerMath
{
	...
	U32 RandomGen::Random(U32 n)
	{
		...
		if(mti >= CMATH_N){//generate N words at a time
			int kk;
			
			if(mti == CMATH_N+1)//if sgenrand() has not been called
				SetRandomSeed(4357); //a default initial seed
	...
	F32 RandomGen::Random(void)
	{
		F32 r = F32(Random(MAXINT));
		F32 divisor = F32(MAXINT);
		return (r / divisor);
	}



	void RandomGen::SetRandomSeed(U32 n)
	{
		//setting initial seeds to mt[N] using
		//generator Line 25 of Table 1 in
	...
	U32 RandomGen::GetRandomSeed(void)
	{
		return(rseed);
	}
